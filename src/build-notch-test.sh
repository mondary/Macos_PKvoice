...
APP_VERSION="${APP_VERSION:-1.14}"

SRC_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "$SRC_DIR/.." && pwd)"
DIST_DIR="$ROOT_DIR/release"
APP_DIR="$DIST_DIR/${APP_NAME}.app"
APP_SRC_DIR="$SRC_DIR/app/cmd/notchtest"
...
echo "Building notch test app... (${APP_VERSION})"
ARCH="${GOARCH:-}"
if [[ -z "$ARCH" ]]; then
  case "$(uname -m)" in
    arm64) ARCH="arm64" ;;
...
(
  cd "$APP_SRC_DIR"
  GOOS=darwin GOARCH="$ARCH" go build -o "$BIN_DIR/pkvoice_notchtest" .
)
...
<dict>
  ...
  <key>CFBundleExecutable</key><string>pkvoice_notchtest</string>
